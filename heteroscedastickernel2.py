...
def hypopth(y, numrestarts, R):
    ...
    for i in range(numrestarts):    
        ...
        k1s4[i] = step4res[0][0]
        k2s4[i] = step4res[0][1]
    lmltest = [lml([k1s4[i],k2s4[i]],y,np.var(y)**0.5) for i in range(numrestarts)]
...
rtest = exp(ystar)
Rtest = rtest*np.identity(n)

numrestarts = 10
#test1,test2 = hypopt(y,10)
test3,test4 = hypopth(y,10,Rtest)
...
def hetloopSK(fmst,varfmst,numiters):
    ...
    while i < numiters:        
        ...
        gpr2 = GaussianProcessRegressor(kernel=kernel2, n_restarts_optimizer = 10, normalize_y=False, alpha=np.var(z))
        gpr2.fit(x, z)
...
yp = np.polyfit(xsig.reshape(np.size(ysam,0)),varsam,5)
pol = np.poly1d(yp)
